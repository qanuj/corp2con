        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if(ModelState.IsValid)
            {
                ...
                if (!(await UserManager.IsEmailConfirmedAsync(user.Id))){
                    var codeConfirm = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
                    var callbackConfirmUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = codeConfirm }, protocol: Request.Url.Scheme);
                    await UserManager.SendEmailAsync(user.Id, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackConfirmUrl + "\">here</a>");
                    return View("ForgotPasswordConfirmation");
                }
